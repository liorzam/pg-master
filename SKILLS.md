...
MCP Connection Failed - If you see "Connection refused":
...
> [!NOTE]
> IMPORTANT: For each finding, highlight the concrete business or performance impact inline. For example: 'Missing index on users.email — adding this will speed up login queries by ~10x' or 'Components re-render on every state change — memoizing here will reduce renders by ~60%'. Be specific about the actual impact based on what you see in the code and what the knowledge recommends.
